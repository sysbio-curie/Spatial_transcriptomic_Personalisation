...
def PlotLRGenes(
    ...
    L_ct_index = generated_cells_vis_L.obs["discrete_label_ct"].isin(
        np.array([celltype_L])
    )

    generated_cells_vis_R = generated_cells.copy()
    R_ct_index = generated_cells_vis_R.obs["discrete_label_ct"].isin(
        np.array([celltype_R])
    )
...
class ConfigWrapper(object):
    ...
    def to_dict_type(self):
        return {
            key: (
                value if not isinstance(value, ConfigWrapper) else value.to_dict_type()
            )
...
def removeOutliers(ad_sc_1k, cell_type, deviation=3):
    ...
    for c in cell_types:
        ...
        valid_cells += list(
            ad_sc_1k.obs.loc[ad_sc_1k.obs[cell_type] == c].index.values[
                tmp_norm < np.median(tmp_norm) * deviation
            ]
        )
...
def ComputeSigma(sc_data_process_marker, cell_class_column):
    if "Marker" in sc_data_process_marker.var.columns:
        sc_data_process_marker = sc_data_process_marker[
            :, sc_data_process_marker.var["Marker"]
        ]
    df_category = (
        sc_data_process_marker.obs[[cell_class_column]]
        .astype("category")
...
def DissectSegRes(df):
    ...
    for row in df.iterrows():
        ...
        for idx, i in enumerate(row[1]["segmentation_centroid"]):
            tmps.append(
                list(i)
                + [row[0], row[0] + "_{}".format(idx), row[1]["segmentation_label"]]
            )
    return pd.DataFrame(
        tmps, columns=["x", "y", "spot_index", "cell_index", "cell_nums"]
    )
...
def PlotVisiumCells(
    ...
    **kwargs,
    ...
    if show_circle:
        ...
        sf = adata.uns["spatial"][list(adata.uns["spatial"].keys())[0]]["scalefactors"][
...
def PlotVisiumGene(
    ...
    if not "spatial" in test.obsm.keys():
        if "x" in test.obs.columns:
            ...
            raise ValueError("No locations founded in obs")
        spot_size = 30
    else:
        spot_size = None
...
def PlotVisiumCellType(ad_sp, weight, cell_type, size=1.5, ax=None):
    ...
    test.obs[cell_type] = (test.obs[cell_type] - test.obs[cell_type].min()) / (
        test.obs[cell_type].max() - test.obs[cell_type].min()
    )
...
def GenerateCellTypeProportion(ss_res):
    ss_res = pd.pivot_table(
        ss_res[["spot_index", "discrete_label_ct"]],
        index=["spot_index"],
        columns=["discrete_label_ct"],
        aggfunc=len,
        fill_value=0,
    ).reset_index()
    ss_res = ss_res.set_index("spot_index")
    ss_res = pd.DataFrame(
        ss_res.values / ss_res.values.sum(1)[:, None],
        columns=ss_res.columns,
        index=ss_res.index,
    )
    return ss_res
...
def ConcatCells(
    spot_id_range,
    file_path="/home/share/xwanaf/sour_sep/Visium/data",
    prefix="",
    suffix="_noMarker_heart_Sig1.6_1e-05.h5ad",
    obs=None,
):
    slices = []
    for spot_id in np.concatenate(
        (spot_id_range[:-1][None, :], spot_id_range[1:][None, :]), axis=0
    ).T:
        sep = "_"
    ...
    y_sep_adata = slices[0].concatenate(
        slices[1:], batch_key=None, uns_merge="unique", index_unique=None
    )
...
def PlotSampledData(
    gen_sample_adata, sc_data_process, cell_type_key, palette=None, scale=True
):
    ...
    gen_sample_adata.obsm["X_pca"] = (
        gen_sample_adata.X - adata_snrna_raw_small_raw.X.mean(0)
    ) @ pcs
    ...
    sc.tl.umap(adata_all, min_dist=0.5, spread=1.0, maxiter=80)

    df = pd.DataFrame(adata_all.obsm["X_umap"], columns=["x", "y"])
    color_mu = [
        "All cells from the single-cell reference data"
        for i in range(adata_all.shape[0] - gen_sample_adata.shape[0])
    ] + ["Generated cells by SpatialScope" for i in range(gen_sample_adata.shape[0])]
    df["Label"] = color_mu

    fig, ax = plt.subplots(figsize=(10, 10), dpi=150)
    sns.scatterplot(
        data=df, x="x", y="y", hue="Label", s=2, palette=["#a1c9f4", "#c44e52"], ax=ax
    )
...
def construct_obs_plot(df_plot, adata, perc=0, suffix=None):
    ...
    df_plot = df_plot.clip(
        df_plot.quantile(perc), df_plot.quantile(1 - perc / 3), axis=1
    )
...
def VisualGenesSc(sp_res, adata_Mop, genes, **kwargs):
    ...
    adata_Mop_ori = AnnData(
        X=adata_Mop[adata_Mop.obs.index.isin(barcodes)].X, var=adata_Mop.var
    )
    adata_Mop_ori.obs["X"] = adata_Mop[adata_Mop.obs.index.isin(barcodes)].obsm[
        "spatial"
    ][:, 0]
    adata_Mop_ori.obs["Y"] = adata_Mop[adata_Mop.obs.index.isin(barcodes)].obsm[
        "spatial"
    ][:, 1]
    ...
    plot_genes_sc(
        genes,
        adata_measured=adata_Mop_ori,
        adata_predicted=adata_Mop_pre,
        x="X",
        y="Y",
        spot_size=270,
        scale_factor=0.1,
...
def reorder(
    file1, file2, spot_id_name="spot_index", cell_type_colname="cell_type_label"
):
    file1 = file1.copy()
    file2 = file2.copy()
    ...
    for i, spot_id in enumerate(np.unique(file2.obs[spot_id_name])):
        ...
        for j, val in enumerate(vals):
            ...
            d_mtx = cosine_similarity(
                file2_spot_id.X[ct_index], file1_spot_id.X[ct_index]
            )
            ...
            spot_reorder_index[ct_index] = np.arange(file1.shape[0])[spot_id_index][
                ct_index
            ][c_index]
...
def add_spot_label(
    spatial_adata, x_seg, y_seg, location_X_column="X", location_Y_column="Y"
):
    ...
    for i in range(x_seg.shape[0] - 1):
        for j in range(y_seg.shape[0] - 1):
            spot_ind = (
                (location_X >= x_seg[i])
                & (location_X <= x_seg[i + 1])
                & (location_Y >= y_seg[j])
                & (location_Y <= y_seg[j + 1])
            )
...
def generate_spot_adata_func(
    spatial_adata, sample_UMI=520
):  # spatial_adata is in count scale
    ...
    def C_sample(a, sample_UMI):
        ...
        if a.sum(0).squeeze().astype(int).sum() < int(sample_UMI) + 10:
            ...
            ball = [
                [i] * a.sum(0).squeeze().astype(int)[i]
                for i in range(data_exp.shape[1])
            ]
            choose_a = [item for sublist in ball for item in sublist]
            subsample_unique = np.random.choice(
                a=choose_a, size=int(sample_UMI), replace=False
            )
    ...
    generated_spot_adata = AnnData(
        X=np.concatenate(generated_spot), var=spatial_adata.var, obs=generated_spot_obs
    )
...
def rename_cell_locations_obs(spatial_adata):
    reorder_ind = []
    for i in np.unique(spatial_adata.obs["spot_index"]):
        reorder_ind = reorder_ind + list(
            np.where(spatial_adata.obs["spot_index"] == i)[0]
        )
    ...
    temp_obs = temp_obs.rename(
        columns={"spot_index": "spot_index_int", "spot_label": "spot_index"}
    )
    return temp_obs
