...
class SpatialScopeNS:
    ...
    def LoadData(self, ST_Data, Img_Data, min_counts, max_counts):
        ...
        if ext in [".tif", ".tiff"]:
            ...
            slide = openslide.OpenSlide(Img_Data)
            region = slide.read_region((0, 0), level=1, size=slide.level_dimensions[1])
            image = np.array(region)[:, :, :3]
        else:
    ...
    def NucleiSegmentation(self):
        ...
        self.loggings.info(
            f"Number of segments: {len(np.unique(self.image['segmented_stardist_default']))}"
        )
        ...
        for row in df_cells.iterrows():
            if row[1]["segmentation_label"] > self.max_cell_number:
                ...
                df_cells.loc[row[0], "segmentation_centroid"] = row[1][
                    "segmentation_centroid"
                ][: self.max_cell_number]
        self.sp_adata.obsm["image_features"] = df_cells

        self.sp_adata.obs["cell_count"] = self.sp_adata.obsm["image_features"][
            "segmentation_label"
        ].astype(int)
        ...
        self.image.show(
            "segmented_stardist_default", cmap="jet", interpolation="none", ax=axes[1]
        )
        ...
        self.sp_adata.uns["cell_locations"] = self.DissectSegRes(
            self.sp_adata.obsm["image_features"]
        )
        self.sp_adata.obsm["image_features"]["segmentation_label"] = self.sp_adata.obsm[
            "image_features"
        ]["segmentation_label"].astype(int)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--Img_Data", type=str, help="H&E stained image data path", default=None
    )
    parser.add_argument(
        "--prob_thresh",
        type=float,
        help="object probability threshol, decrease this parameter if too many nucleus are missing",
        default=0.2,
    )
    parser.add_argument(
        "--max_cell_number", type=int, help="maximum cell number per spot", default=20
    )
    parser.add_argument(
        "--min_counts", type=int, help="minimum UMI count per spot", default=500
    )
    parser.add_argument(
        "--max_counts", type=int, help="maximum UMI count per spot", default=50000
    )
